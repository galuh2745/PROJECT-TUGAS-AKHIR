...
export default function BarangKeluarDagingPage() {
  ...
  const [formHeader, setFormHeader] = useState({ tanggal: '', customer_id: '', pengeluaran: '', keterangan: '', jumlah_bayar: '', metode_pembayaran: 'CASH', tipe_pembayaran: 'hutang' as 'lunas' | 'sebagian' | 'hutang', is_harga_bis: false, harga_bis: '' });
  ...
  const openAddModal = () => {
    ...
    setFormHeader({ tanggal: localDate, customer_id: '', pengeluaran: '', keterangan: '', jumlah_bayar: '', metode_pembayaran: 'CASH', tipe_pembayaran: 'hutang' as 'lunas' | 'sebagian' | 'hutang', is_harga_bis: false, harga_bis: '' });
  ...
  const openEditModal = (item: BarangKeluarDaging) => {
    ...
    setFormHeader({ tanggal: item.tanggal, customer_id: matchedCustomer?.id || '', pengeluaran: item.pengeluaran.toString(), keterangan: item.keterangan || '', jumlah_bayar: tipePembayaran === 'sebagian' ? (item.jumlah_bayar ?? 0).toString() : '', metode_pembayaran: 'CASH', tipe_pembayaran: tipePembayaran, is_harga_bis: false, harga_bis: '' });
  ...
  const calculatedTotal = useMemo(() => {
    const base = formDetails.reduce((s, d) => s + d.subtotal, 0);
    const biayaBis = (formHeader.is_harga_bis && formHeader.harga_bis) ? (parseFloat(formHeader.harga_bis) || 0) : 0;
    return base + biayaBis;
  }, [formDetails, formHeader.is_harga_bis, formHeader.harga_bis]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const body = { ...(modalMode === 'edit' && { id: selectedData?.id }), tanggal: formHeader.tanggal, customer_id: formHeader.customer_id, nama_customer: selectedCustomer?.nama || '', pengeluaran: parseFloat(formHeader.pengeluaran) || 0, keterangan: formHeader.keterangan, jumlah_bayar: calculatedJumlahBayar, metode_pembayaran: formHeader.metode_pembayaran, total_penjualan_custom: formHeader.is_harga_bis ? (parseFloat(formHeader.harga_bis) || 0) : undefined, details: formDetails.map(d => ({ jenis_daging_id: d.jenis_daging_id, berat_kg: parseFloat(d.berat_kg) || 0, harga_per_kg: parseFloat(d.harga_per_kg) || 0 })) };
            ...
            {customerList.find(c => c.id === formHeader.customer_id)?.nama?.toLowerCase().includes('bagus') && (
              <div className="rounded-md border border-emerald-200 bg-emerald-50/50 p-4 space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <button type="button" onClick={() => setFormHeader({ ...formHeader, is_harga_bis: !formHeader.is_harga_bis, harga_bis: '' })}
                    className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${formHeader.is_harga_bis ? 'bg-emerald-600' : 'bg-gray-300'}`}>
                    <span className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${formHeader.is_harga_bis ? 'translate-x-6' : 'translate-x-1'}`} />
                  </button>
                </div>
                {formHeader.is_harga_bis && (
                  <div className="space-y-2">
                    <Label className="text-xs">Harga Bis <span className="text-red-500">*</span></Label>
                    <Input type="number" placeholder="Masukkan harga bis" value={formHeader.harga_bis} onChange={(e) => setFormHeader({ ...formHeader, harga_bis: e.target.value })} min={1} required />
